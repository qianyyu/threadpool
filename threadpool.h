...
typedef struct {
    ...
    ThreadPool_work_queue_t* queue_task;
...
#endif
/** Dropped Code:
 *     bool cmp = [](ThreadPool_work_t left, ThreadPool_work_t right) { 
        struct stat sb;
        struct stat sb2;
        stat((const char*) left.arg, &sb);
        stat((const char*) right.arg, &sb2);
        return (sb.st_size ) < (sb2.st_size); };
